...
if ($Test) {
    ...
    # A frame that overruns lasts two fields, so the field is the shortest
    ...
    Write-Host ('Frame cost: {0:n0} cycles on average, {1:n0} at most, of a {2:n0}-cycle field ({3:p0} at most)' -f `
        $busy.Average, $busy.Maximum, $field, ($busy.Maximum / $field))
    if ($late -or $busy.Maximum -gt $field) {
        Write-Warning "$late of $($perFrame.Count) frames took longer than a field, so the rain misses 60 fps."
    }
    return
}
