; ============================================================
; Matrix Rain Effect (Commodore 4032)
; ============================================================
; Matrix-style falling characters
; Modified for 40-column PET (4032)
...
;
; Memory Map:
;   Screen RAM: $8000-$83E7 (40x25 = 1000 bytes)
...
SCREEN_HI    = $80
SCREEN_END   = $84              ; screen ends before $8400
...
NextColumn:
        ...
        ; Bounds check: must be in screen range $8000-$83FF
        lda $fe
        cmp #SCREEN_HI
        bcs +                  ; >= $80, check upper bound
        jmp SkipColumn         ; < $8000, not on screen yet
+       cmp #SCREEN_END
        bcc +                  ; < $84, in valid range
        jmp SkipColumn         ; >= $8400, off screen
+
...
.writeHead:
        ...
        lda $fe
        cmp #$83               ; bottom line is $83C0-$83E7
        bne .doWrite
        lda $fd
        cmp #$C0               ; >= $83C0?
        bcc .doWrite
...
; ------------------------------------------------------------
        ...
        ; Calculate address one row up (subtract 40)
        lda $fd
        sec
        sbc #COLS
        sta $fb
...
; ------------------------------------------------------------
        ...
        ; Calculate position: head - (row * 40)
        sta $f7                ; save row number
        jsr MultiplyBy40       ; result in $f9/$fa
...
SkipTail:
        ; Move down one row (add 40 to position)
        lda RAINLO,x
        clc
        adc #COLS
        sta RAINLO,x
...
RAINHI   !fill 40,0            ; current Y position (high byte)
SPEED    !fill 40,0            ; movement delay (0=fastest)
DEL      !fill 40,0            ; current delay counter
TRAIL    !fill 40,0            ; trail length for this column
...
TRAILMIN !byte 14              ; minimum trail length (rows)
...
NUMDRIPS !byte 40              ; active columns (max 40, try 35 for gaps)
SPDSTART !byte 15              ; initial speed range (0 to N-1, lower=faster)
SPDRESET !byte 8               ; reset speed range (0 to N-1, lower=faster)
